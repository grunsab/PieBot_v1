...
# (Implementations remain the same, initialization handled in TitanMini)
# ==============================================================================
...
class RelativePositionBias(nn.Module):
    def __init__(self, num_heads, max_relative_position=7):
        ...
        # Initialization handled in _init_parameters of TitanMini
...
# ==============================================================================
# 4. Main Model: Titan-Mini (Integrated Architecture)
# ==============================================================================

class TitanMini(nn.Module):
    """
    Titan-Mini Transformer network integrating optimized input handling:
    Symmetric Embeddings, Global Context Injection, Material-aware CLS token,
    ...
    def __init__(
        ...
        self.piece_type_embedding = None
        ...
        if self.use_piece_embeddings:
            # --- Strategy for Sparse Inputs (Integrated Approach) ---
            if input_planes < 12:
                 raise ValueError(f"Sparse input mode requires at least 12 planes, got {input_planes}")

            # 1. Symmetric Embeddings (Fix 1)
            # Based on encoder.py order: P, R, B, N, Q, K
            ...
            # 2. Global Context Projection (Fix 2)
            num_global_features = input_planes - 12
            if num_global_features > 0:
                # Used for both CLS token initialization and Board Token enhancement
                self.global_feature_proj = nn.Linear(num_global_features, d_model)

            # 3. Input Stabilization (LayerNorm on the input embeddings)
        ...
        # Initialize parameters (Fix 4)
        self._init_parameters()
        self._init_head_specific_weights()

    def _process_sparse_input(self, x):
        """
        Converts sparse planes into symmetric embeddings (Type + Color).
        Returns: (B, 64, d_model) tokens, (B, 64) occupancy mask.
        """
        B, C, H, W = x.shape
        ...
        # 2. Identify Piece Types (Symmetric)
        ...
        # Empty=0, P=1, R=2, B=3, N=4, Q=5, K=6
        ...
        return board_tokens, is_occupied.float()
        
    def _init_parameters(self):
        """
        FIX 4: Initialize parameters using Standardized Truncated Normal (std=0.02).
        """
        std_dev = 0.02

        # Helper function for Truncated Normal (robust across PyTorch versions)
        ...
        # Initialize specific parameters if they exist
    ...
    def _init_head_specific_weights(self):
        # Initialize final layers of the heads with a smaller std deviation 
        # (10x smaller than base) to prevent saturation early in training.
    ...
    def forward(self, x, value_target=None, policy_target=None, policy_mask=None, 
        ...
        if self.use_piece_embeddings:
            ...
            # 1a. Generate Symmetric Embeddings (Fix 1)
            tokens_symmetric, is_occupied = self._process_sparse_input(x) # [B, 64, d_model], [B, 64]

            # 1b. Handle Global Features (Fix 2)
            global_proj = torch.zeros(batch_size, self.d_model, device=x.device, dtype=x.dtype)
            if self.global_feature_proj is not None:
                # Global features (Planes 12+) are constant; take from (0,0).
                global_features = x[:, 12:, 0, 0]
                global_proj = self.global_feature_proj(global_features) # [B, d_model]

            # 1c. Material Injection into CLS (Fix 3)
            # Calculate Average Material Embedding (only average actual pieces)
            
            # Ensure robust calculation for AMP (float32 accumulation)
            pooling_dtype = torch.float32 if tokens_symmetric.dtype == torch.float16 else tokens_symmetric.dtype

            is_occupied_mask = is_occupied.to(pooling_dtype).unsqueeze(-1) # [B, 64, 1]
            occupied_count = torch.sum(is_occupied_mask, dim=1) # [B, 1]
            occupied_count = torch.clamp(occupied_count, min=1.0) # Prevent div by zero

            material_sum = torch.sum(tokens_symmetric.to(pooling_dtype) * is_occupied_mask, dim=1) # [B, d_model]
            material_avg = material_sum / occupied_count # [B, d_model]
            
            # Cast back to required dtype
            material_avg = material_avg.to(tokens_symmetric.dtype)

            # Combine CLS components (Material + Global Context)
            cls = (material_avg + global_proj).unsqueeze(1) # [B, 1, d_model]

            # 1d. Enhance Board Tokens with Global Context (Fix 2)
            board_tokens = tokens_symmetric + global_proj.unsqueeze(1)

            # 1e. Input Stabilization
            ...
            # 1b. Use learned CLS token
            cls = self.cls_token.expand(batch_size, -1, -1)

        
        # 2. Add positional encodings to board tokens.
        # Positional encoding is added to board tokens, but not the CLS token.
...
if __name__ == "__main__":
    # --- Model Initialization Test (Sparse Mode - 16 planes) ---
    print("--- Testing TitanMini (Integrated Architecture - Sparse 16-plane Mode) ---")
    # Set seed for reproducible initialization check
    torch.manual_seed(42) 
    model_sparse = TitanMini(use_wdl=True, input_planes=16, num_layers=2)
    ...
    # Simulate placing pieces based on encoder.py definitions:
    # Plane 0=WPawn (Friendly), Plane 6=WKnight (Friendly), Plane 7=BKnight (Enemy)
    dummy_input_sparse[0, 0, 4, 4] = 1 
    dummy_input_sparse[0, 6, 2, 2] = 1 
    dummy_input_sparse[0, 7, 2, 5] = 1 
    
    # Batch 1: Only Kings (Plane 10=WKing, Plane 11=BKing) - tests empty board handling for pooling
    dummy_input_sparse[1, 10, 0, 0] = 1
    dummy_input_sparse[1, 11, 7, 7] = 1

    # Simulate global features (Planes 12-15)
    dummy_input_sparse[:, 12, :, :] = 1 # e.g., White Kingside Castling active
    ...
    # Verification of the symmetric embedding process
    print("\nVerifying symmetric embedding process...")
    with torch.no_grad():
        tokens, _ = model_sparse._process_sparse_input(dummy_input_sparse)
        # Knight Type Index = 4 (P=1, R=2, B=3, N=4). Color 0=Friendly, 1=Enemy.
        # WKnight (Type 4 + Color 0)
        w_knight_emb = model_sparse.piece_type_embedding(torch.tensor(4)) + model_sparse.color_embedding(torch.tensor(0))
        # BKnight (Type 4 + Color 1)
        b_knight_emb = model_sparse.piece_type_embedding(torch.tensor(4)) + model_sparse.color_embedding(torch.tensor(1))
        
        # Index for (2,2) is 2*8 + 2 = 18. Index for (2,5) is 2*8 + 5 = 21.
        if torch.allclose(tokens[0, 18], w_knight_emb, atol=1e-6) and torch.allclose(tokens[0, 21], b_knight_emb, atol=1e-6):
            print("Symmetric embeddings verified successfully.")
        else:
            print("ERROR: Symmetric embeddings verification failed!")
    ...
    print(f"\nSparse Mode Loss: {total_loss.item():.4f}")

    # Check initialization stability
    model_sparse.eval()
    with torch.no_grad():
        value, _ = model_sparse(dummy_input_sparse)
        print(f"Initial value outputs: {value.squeeze().detach().numpy()}")
        if torch.max(torch.abs(value)) > 0.2:
             print("INFO: Initial values seem somewhat large.")
        else:
             print("INFO: Initial values are small, standardized initialization seems effective.")

    # --- Test Dense Mode ---
    print("\n--- Testing TitanMini (Dense 112-plane Mode) ---")
    model_dense = TitanMini(use_wdl=True, input_planes=112, num_layers=2)
