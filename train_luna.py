...
def create_data_loaders(args):
    ...
    val_loader = DataLoader(
        val_dataset,
        batch_size=max(1, batch_size * 2),  # Larger batch for validation
        shuffle=False,
        num_workers=num_workers,
        pin_memory=(device.type == 'cuda'),
        persistent_workers=True if num_workers > 0 else False
...
def train_epoch(model, train_loader, optimizer, scheduler, scaler, args, epoch, writer, ema=None):
    """Train for one epoch."""
    model.train()
...
def validate(model, val_loader, args, epoch, writer):
    """Validate the model."""
    model.eval()
...
def train():
    ...
    # Setup EMA
    ema = EMA(model, decay=0.9999) if args.swa else None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_value_loss, train_policy_loss = train_epoch(
            model, train_loader, optimizer, scheduler, scaler, args, epoch, writer, ema
        )
        ...
        val_loss, val_value_loss, val_policy_loss = validate(
            model, val_loader, args, epoch, writer
        )
        ...
        if (epoch + 1) % args.save_every == 0:
            checkpoint_path = os.path.join(
                args.checkpoint_dir,
                f'luna_epoch_{epoch + 1}.pt'
            )
            save_checkpoint(model, optimizer, scheduler, epoch, args, best_val_loss, checkpoint_path)
        
        # Save best model
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_path = os.path.join(args.checkpoint_dir, 'luna_best.pt')
            save_checkpoint(model, optimizer, scheduler, epoch, args, best_val_loss, best_path)
            print(f'New best validation loss: {best_val_loss:.4f}')
        
        # Stochastic Weight Averaging
        if args.swa and epoch >= args.swa_start and ema:
            ema.apply_shadow()
            swa_val_loss, _, _ = validate(model, val_loader, args, epoch, writer)
            ...
            if swa_val_loss < best_val_loss:
                ...
                save_checkpoint(model, optimizer, scheduler, epoch, args, best_val_loss, swa_path)
