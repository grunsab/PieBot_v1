...
class UltraPerformanceMCTSEngine:
    ...
    def search(self, board, num_simulations):
        ...
        if root_hash in self.node_lookup:
            ...
            if self.verbose:
                ...
                print(f"Reusing existing tree with root at index {root_idx}, visits={root.visits}")
    ...
    def _leaf_collector(self, board, target_simulations):
        ...
        while True:
            ...
            if root_fen not in self.node_lookup:
                ...
                # But decrement counter since we didn't actually do the simulation
                with self.simulation_lock:
                    self.total_simulations -= 1
                continue
            ...
            while True:
                ...
                if sim_board.is_game_over():
                    ...
                    terminal_value = value / 2.0 + 0.5
                    break
                
                # Check if leaf
                if node_idx not in self.children:
                    # Found a leaf - add to buffer for expansion
                    leaf_found = True
                    with self.buffer_lock:
                ...
                if not children:
                    # This shouldn't happen - expanded nodes should have children
                    # But if it does, treat as terminal with draw value
                    terminal_value = 0.5
                    break
                
                best_child_idx = self._select_child_with_virtual_loss(children)
                if best_child_idx < 0 or best_child_idx >= len(self.nodes):
                    # Invalid child index - treat as terminal
                    terminal_value = 0.5
                    break
                ...
                if best_child.move in sim_board.legal_moves:
                    ...
                    # Move is invalid - this shouldn't happen in a properly
                    # constructed tree, but handle it gracefully
                    terminal_value = 0.5
                    break
            
            # If we found a terminal value but no leaf, backup immediately
            if terminal_value is not None and not leaf_found:
                self._backup(path, terminal_value, remove_virtual_loss=True)
    ...
    def _select_child_with_virtual_loss(self, children_indices):
        ...
        # Add some randomness to tie-breaking for diversity
        import random
        ...
        for child_idx in valid_children:
            ...
            if visits_with_vl > 0:
                ...
                # Unvisited nodes: use optimistic initialization
                # This encourages exploration of unvisited nodes
                q_value = 0.5
    ...
    def _select_best_move(self, root_idx, temperature=0):
        ...
        # CRITICAL FIX: Ensure all children have at least one visit
        # This prevents selecting moves with 0 visits when reusing the tree
        unvisited_children = []
        for child_idx in children:
            if child_idx < len(self.nodes):
                child = self.nodes[child_idx]
                if child.visits == 0:
                    unvisited_children.append((child_idx, child.move))
        
        # If there are unvisited children, select one randomly to ensure diversity
        if unvisited_children and len(unvisited_children) < len(children) // 2:
            # Only do this if less than half the children are unvisited
            # Otherwise we haven't explored enough
            import random
            selected_idx, selected_move = random.choice(unvisited_children)
            if self.verbose:
                print(f"Warning: Selected unvisited move {selected_move} from {len(unvisited_children)} unvisited children")
        ...
        if temperature == 0:
            ...
            # Don't select moves with 0 visits
            if best_visits == 0:
                if self.verbose:
                    print("Warning: All children have 0 visits, cannot select move")
                return None
            ...
            for child_idx in children:
                if child_idx < len(self.nodes):
                    child = self.nodes[child_idx]
                    # Only include children with visits > 0
                    if child.visits > 0:
